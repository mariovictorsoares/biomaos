    ...
    <div class="flex min-h-full items-center justify-center p-2 sm:p-4">
      <div class="relative w-full max-w-md glass-panel rounded-xl shadow-xl transform transition-all max-h-[90vh] flex flex-col">
        ...
        <div class="flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
          ...
          <div class="min-w-0">
            <h2 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">Embalagens</h2>
            <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Gerenciar embalagens</p>
          </div>
        </div>
        ...
        <div class="px-4 sm:px-6 py-4 sm:py-6 overflow-y-auto flex-1">
          ...
          <!-- Lista -->
          <div v-if="embalagens.length > 0" class="space-y-2">
            <div
              v-for="item in embalagens"
              :key="item.id"
          ...
          <div v-else class="text-center py-6 sm:py-8 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
            ...
            <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Nenhuma embalagem cadastrada</p>
          </div>
        </div>
...
const emit = defineEmits(['close', 'nova-embalagem', 'editar-embalagem'])
