<template>
  <div>
    <h1 class="text-xl sm:text-2xl font-bold text-text-light dark:text-text-dark mb-4 sm:mb-6">Produção</h1>
      ...
      <div class="p-3 sm:p-4 border-b border-border-light dark:border-border-dark">
        <div class="flex flex-col gap-3">
          ...
          <div class="flex items-center justify-between">
            <h2 class="text-xs sm:text-sm font-medium text-subtext-light dark:text-subtext-dark uppercase tracking-wider">Lista Produção</h2>
            <!-- Botão Desktop -->
          ...
          <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
            ...
            <div class="flex items-center gap-2">
              ...
              <div class="flex items-center">
                ...
                <div class="px-2 sm:px-3 py-1.5 sm:py-2 border-y border-border-light dark:border-border-dark bg-white dark:bg-gray-800 text-[11px] sm:text-sm min-w-[140px] sm:min-w-[200px] text-center whitespace-nowrap">
            ...
            <!-- Filtros por Etapa -->
            <div class="flex items-center gap-1 overflow-x-auto pb-1 -mx-3 px-3 sm:mx-0 sm:px-0 sm:pb-0">
              <button
        ...
        <div class="table-scroll-container custom-scrollbar" ref="tableScrollRef">
          <table class="w-full text-left border-collapse" style="min-width: 1600px;">
              <thead>
                ...
                <tr>
                  <th colspan="8" class="bg-[#C4A574] text-white text-center py-2 text-xs font-medium border-r border-[#B39560]">
                    Planejado
                  </th>
                ...
                <!-- Subheader -->
                <tr class="bg-gray-50 dark:bg-gray-800 border-b border-border-light dark:border-border-dark text-xs">
                  <!-- Cod e Fazenda -->
                  ...
                  <th class="table-header font-medium text-center whitespace-nowrap">Quantia</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Bandejas</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Plantio</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Luz</th>
                  <th class="table-header font-medium text-center border-r border-border-light dark:border-border-dark whitespace-nowrap">Colheita</th>
                  <!-- Realizado -->
                  <th class="table-header font-medium text-center whitespace-nowrap">Plantio</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Quantia</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Luz</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Quantia</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Perda Luz</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Colheita</th>
                  <th class="table-header font-medium text-center whitespace-nowrap">Quantia</th>
              ...
              <tbody class="divide-y divide-border-light dark:divide-border-dark">
                ...
                >
                  ...
                  <!-- Planejado -->
                  <td class="table-cell text-xs font-medium whitespace-nowrap">{{ prod.especies?.codigo || '-' }}</td>
                  ...
                  <td class="table-cell text-center p-1" @click.stop>
                    <input
                      ...
                      class="w-[100px] text-center text-xs py-1.5 px-1 border border-gray-200 dark:border-gray-600 rounded bg-white dark:bg-gray-800 focus:ring-1 focus:ring-primary focus:border-primary"
                    />
                  </td>
                  ...
                  <td class="table-cell text-center p-1" @click.stop>
                    <input
                      ...
                      :class="[
                        'w-[100px] text-center text-xs py-1.5 px-1 border rounded focus:ring-1 focus:ring-primary focus:border-primary',
                  ...
                  <!-- Perda Luz - calculado automaticamente -->
                  <td class="table-cell text-center text-xs">{{ calcularPerdaLuz(prod) }}</td>
                  ...
                  <!-- Perda Colheita - calculado automaticamente -->
                  <td class="table-cell text-center text-xs">{{ calcularPerdaColheita(prod) }}</td>
        ...
        >
          ...
          <div class="flex items-center gap-3 mb-3">
            <div class="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
              <span class="text-sm font-bold text-primary">{{ prod.especies?.codigo?.substring(0, 2) || '-' }}</span>
            </div>
            <div class="min-w-0 flex-1">
              <p class="font-semibold text-sm text-text-light dark:text-text-dark truncate">{{ prod.especies?.nome || '-' }}</p>
                  ...
                  <div class="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100 dark:border-border-dark flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                      <div class="w-10 h-10 sm:w-14 sm:h-14 rounded-xl bg-primary/10 flex items-center justify-center text-sm sm:text-lg font-bold text-primary overflow-hidden shrink-0">
                        {{ selectedProducao?.especies?.codigo?.substring(0, 2) || 'PR' }}
                      </div>
                      <div class="min-w-0 flex-1">
                        <h2 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-text-dark truncate">{{ selectedProducao?.especies?.nome || '-' }}</h2>
                      ...
                      <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 sm:p-5">
                        ...
                        <div class="grid grid-cols-2 gap-3 sm:gap-4">
                          ...
                          <div>
                            ...
                            <p class="text-sm font-medium text-gray-900 dark:text-text-dark">{{ selectedProducao?.especies?.codigo || '-' }}</p>
                          </div>
                        </div>
                      </div>
...
interface Lote { id: string; numero: string; especie_id?: string; tempo_germinacao?: number; tempo_luz?: number; created_at?: string }
...
const filteredProducao = computed(() => {
  ...
  result = result.filter(p => {
    ...
    // "Todos" - mostra planos que têm qualquer data no período
    return (dp && dp >= inicio && dp <= fim) || (dl && dl >= inicio && dl <= fim) || (dc && dc >= inicio && dc <= fim)
  })
...
async function updateRealizadoPlantioQtd(prod: Producao, event: Event) {
  ...
  // Se quantidade igual ou maior, salva direto
  try {
    const updateData: Record<string, any> = {
...
async function loadProducao() {
  ...
  try {
    const { data, error } = await supabase
      .from('producao')
      .select(`*, fazendas:fazenda_id (id, nome), especies:especie_id (id, codigo, nome, tempo_germinacao, tempo_luz), lotes_sementes:lote_id (id, numero)`)
      .eq('empresa_id', currentCompany.value.id)
...
async function loadLotes() {
  if (!currentCompany.value?.id) return
  const { data } = await supabase.from('lotes_sementes').select('id, numero, especie_id, tempo_germinacao, tempo_luz, created_at').eq('empresa_id', currentCompany.value.id).eq('status', 'ativo').order('created_at', { ascending: false })
  lotes.value = data || []
}
...
async function salvarPlano() {
  ...
  try {
    ...
    // Quantidade de planos a criar (1 normal ou 6 se recorrente)
    const totalPlanos = novoPlano.value.recorrente ? 6 : 1
    ...
    const msg = novoPlano.value.recorrente
      ? `Recorrência criada! ${totalPlanos} planos cadastrados.`
